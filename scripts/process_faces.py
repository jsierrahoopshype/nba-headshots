...
def process_player(nba_id, new_only=False):
    """Process a single player headshot."""
    src = os.path.join(ORIGINAL_DIR, f"{nba_id}.png")
    face_out = os.path.join(FACE_DIR, f"{nba_id}.png")
    thumb_out = os.path.join(THUMB_DIR, f"{nba_id}.png")
    ...
    try:
        ...
        log(f"Error processing {nba_id}: {e}")
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument("--id", type=int, default=None, help="Process a single player by NBA ID")
    ...
    if args.id:
        ok = process_player(args.id, new_only=args.new_only)
        log(f"Player {args.id}: {'OK' if ok else 'FAILED'}")
    else:
        files = [f for f in os.listdir(ORIGINAL_DIR) if f.endswith(".png")]
        log(f"Processing {len(files)} headshots...")
        success = 0
        for i, fname in enumerate(files):
            nba_id = fname.replace(".png", "")
            if process_player(nba_id, new_only=args.new_only):
                success += 1
            if (i + 1) % 100 == 0:
